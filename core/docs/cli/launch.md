...
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
| a `file()` field with no `FILES_DIR`                   | the write root is a deploy fact, not a declaration                                         | set `FILES_DIR` — required even when bytes go to an off-box driver                                             |
...
| a gateway entry with an unset or unparseable `APP_URL` | there is no host to grant                                                                  | set `APP_URL` to the app's absolute internal base url                                                          |
