 ...
 *  subscriber expects), and `oldestSupported` (retention guard's floor, 05-runtime.md §event-surface). */
...
/** Decide how to handle a stored payload at `storedVersion` against the chain (05-runtime.md §event-surface retention
...
/** Wrap a `DeliveredMsg` payload through its topic's upcaster chain before consume (05-runtime.md §event-surface) — the
