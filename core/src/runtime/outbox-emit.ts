...
// ─── producer-side backpressure (05-runtime.md §relay §backpressure) ───────────────────────────────────
...
/**
 * Per-consumer fan-out drain (05-runtime.md §relay): fans each message to the consumers `plan(msg)` returns,
...
/**
 * Wrap one consumer invocation in a `consume:<topic>` span (05-runtime.md §relay). Reads the row's
...
/**
 * A per-consumer consume invocation (05-runtime.md §relay composite fence). Each matching subscriber/worker
...
/**
 * Dead-letter a message (05-runtime.md §relay: same shape + `dead_at`, `final_error_kind`). Carries the
