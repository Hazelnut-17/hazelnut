...
export function collectModelGuardViolations(
  ...
  for (const { view: v, door } of remotelyReachableViews(views, model)) {
    ...
    out.push({
      ...
      resources: [src.name],
      refuse:
        `policy/read-protected: view '${v.name}' (over '${src.name}') is ${door} but ${gap} — a view is its OWN read door: the source resource's rowPolicy is NOT re-applied to it, so a narrowed resource read and a wide-open view over the same table are served side by side to the same agent. Refusing to boot: give the view a rowPolicy that yields no rows for an anonymous caller and an ownership / scope-value / grant fragment for the rest — rowPolicy: (actor: Actor | null) => actor ? { owner_id: actor.id } : none(), with none/owned/withinScope/relate on "hazelnut/query". If every caller who reaches this tool is MEANT to see the same rows, say so with shared() / shared(<condition>) from "hazelnut/query" — it lowers identically and is the written decision. features:{ scope:true } does NOT discharge this: scope partitions the tenant boundary, never two callers within it. An anonymous caller reaches the policy as a NON-NULL actor holding no claim, so a null-check guarding all() narrows nobody; test it with isAnonymous(actor) ("hazelnut/authz/auth.ts"). Dropping the view's 'mcp' card also closes it — a view with no mcp card is invisible to agents.`,
      warn:
        `[hazelnut] createRouter: view '${v.name}' (over '${src.name}') is ${door} but ${gap} — the source's rowPolicy is NOT re-applied to a view, so this tool serves rows the resource read hides. Give the view a rowPolicy: (actor: Actor | null) => actor ? { owner_id: actor.id } : none(), return shared() / shared(<condition>) when a uniform read is the intent, drop the view's 'mcp' card, or use createApp for the guarded (fail-closed) path.`,
    });
  }
