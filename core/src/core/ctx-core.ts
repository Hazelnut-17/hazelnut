...
import { getCurrentTraceparent } from "./tracing.ts"; // ambient trace-carrier holder homes with the tracer port (05-runtime.md §relay)
...
// ── end-to-end trace propagation: the ambient current-span carrier (05-runtime.md §relay) ──────────
...
/** Build the `_outbox.trace_context` envelope `ctx.emit` stamps (05-runtime.md §relay): the active span's
...
export async function emitStamped(
  ...
  // Per-source emit budget (05-runtime.md §relay §backpressure), keyed by schedulingCapKey and counted
...
/**
 * The boot-configured per-agent scheduling cap (05-runtime.md §multi-replica-scheduling) — the window quota + the store
...
export interface SchedulingCapConfig {
  ...
  /** The emit-verb budget of the same containment family (05-runtime.md §relay §backpressure) — a distinct
...
/** The born-on per-source emit budget floor (05-runtime.md §relay §backpressure): ~50 emits/s sustained per
...
// Throws the cap rejection as a `.kind: "business"` failure (05-runtime.md §multi-replica-scheduling) so an over-cap
 ...
 * and `makeCtx` (05-runtime.md §multi-replica-scheduling) so both paths get an identical surface. It takes the WHOLE origin,
...
export function makeQueueSurface(
  ...
  if (capConfig === null) {
    return {
      ...
      // the one-shot scheduled job (05-runtime.md §multi-replica-scheduling), bound to this tx db and backpressure-gated like
