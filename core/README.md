...
Issues: https://github.com/Hazelnut-17/hazelnut/issues — bug reports and requests are welcome and read.

**Pull requests against this repository cannot be merged.** Everything under `src/` is assembled
one-way from a private development tree, so the next release overwrites it. That is not a policy
about contributions, it is what this repository IS — see CONTRIBUTING.md for how a fix actually
lands.
