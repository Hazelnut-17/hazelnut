 ...
 *  (`examples/license-system/dogfood-coverage.test.ts`) forces a new dogfood or goes RED on a new face. */
 ...
 *  (`examples/license-system/dogfood-coverage.test.ts`) forces a new dogfood or goes RED on a new feature. */
 ...
 *  carriers, plus `idempotency`. `feature-keys-drift.test.ts` binds this so the two rosters can't drift. */
...
/** Does one op value carry the decisions the pipeline dispatches on — a `policy` slot always, plus an
...
export function pathSegmentErr(
  ...
  if (seg) {
    return `decl/path-invalid: resource '${resource}' path '${path}' — ${seg.replace(/^illegal path name '[^']+' — /, "")}`;
  }
